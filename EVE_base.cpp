...
uint8_t Validate_and_Print_Chip_ID(uint32_t Chip_ID)
{
	...
	{
		DBG_STAT("Unexpected chip if of 0x%02X = %s8%02X, code compiled for %s8%02X\n", this_byte, this_byte < 0x14 ? "FT" : "BT",
		    this_byte, EVE_DEVICE < 0x14 ? "FT" : "BT", EVE_DEVICE);
		return_value = 0;
	}
	return (return_value);
}
...
#if (ROBUST_EXECUTION_COMPLETE == 1)
uint16_t Reset_EVE_Coprocessor(void)
{
	...
	uint8_t BT81x_error_string[129];
...
// list. This happens when EVE_REG_CMD_READ matches EVE_REG_CMD_WRITE,
// indicating that all commands have been executed.  We have a local copy of
...
uint16_t Wait_for_EVE_Execution_Complete(uint16_t SW_write_offset)
{
	...
	uint16_t reported_read_address;
	uint16_t reported_write_address;
	...
	do
	{
			...
			if ((reported_read_address != read_address) || (reported_write_address != write_address))
			{
				if (write_address != SW_write_offset)
				{
					DBG_GEEK("Write Mismatch: HDW_R=(%5u,0x%04X) HDW_W=(%5u,0x%04X) != SW_W=(%5u,0x%04X)\n", read_address, read_address,
					    write_address, write_address, SW_write_offset, SW_write_offset);
				}
		...
		// This appears to break at least the dot drawing of the touch calibrate
		// routine. Best to leave it disabled unless you are using the scope to
		// do low-level SPI hardware debugging. Write and read REG_MACRO_0 to
		// debug SPI
		uint32_t readback;
		SET_DEBUG_LED;
		...
		else
		{
			...
			DBG_GEEK("  Pointers: HDW_R=(%5u,0x%04X) HDW_W=(%5u,0x%04X) != SW_W=(%5u,0x%04X)\n", read_address, read_address, write_address,
			    write_address, SW_write_offset, SW_write_offset);
			timeout = 100000;
		}
...
{
	//the quick version
	while (EVE_REG_Read_16(EVE_REG_CMD_READ) != SW_write_offset) {}
	return SW_write_offset;
}
...
const uint8_t Goodix_GT911_Init_Data[GOODIX_GT911_INIT_DATA_LENGTH] PROGMEM = {};
//----------------------------------------------------------------------------
uint16_t EVE_Init_Goodix_GT911(uint16_t FWol)
{
	...
	// BT817 GPIO3 is used to reset GT911
	...
	// BT817 uses GPIO3 to reset GT911
...
const uint8_t Pen_Up_Bug_Fix_Init_Data[PEN_UP_BUG_FIX_INIT_DATA_LENGTH] PROGMEM = {};
//----------------------------------------------------------------------------
...
// in CFA_E_defines.h to create and display a set of
...
void Read_and_Dump_Touch_Matrix(const __FlashStringHelper *message)
{
	...
	for (uint8_t i = 0; i <= 5; i++)
	{
		...
		DBG_GEEK("    (int32_r)0x%08lX%c // [%c] = %s\n", touch_transform[i], i == 5 ? ' ' : ',', i + 'A', float_string);
	}
	DBG_GEEK("    };\n");
}
#endif //(DEBUG_LEVEL==DEBUG_GEEK)
...
#else
	// Resistive touch should be field calibrated once, then the
	// stored matrix can can be stored and sent to the EVE at
	// boot time.
	// In a final application the end user should have the
	// ability to preform a touch calibration which should be
...
{
	// Wake up the EVE
	delay(20); // Wait a few MS before waking the BT817
...
#if (0 != EVE_TOUCH_CAL_NEEDED) || ((EVE_TOUCH_TYPE == EVE_TOUCH_CAPACITIVE) && (EVE_TOUCH_CAP_DEVICE == EVE_CAP_DEV_GT911)) || \
    (0 != EVE_PEN_UP_BUG_FIX)
	...
	// at this early stage of the game. This stub list just shows a black screen.
	...
	// Tell the EVE that it can swap display lists at the next available frame boundary.
...
#if (EVE_TOUCH_TYPE == EVE_TOUCH_CAPACITIVE)
	// Capacitive. Set multi-touch mode. You gots to do this _after_ the calibration.
...
// zlib compressed unified.blob
// Asset Builder\flash_support\bt815_6\unified.blob
const uint8_t flash_blob[320] PROGMEM = {};
#endif // ((EVE_DEVICE == BT815) ||(EVE_DEVICE == BT816))

#if ((EVE_DEVICE == BT817) || (EVE_DEVICE == BT818))
// zlib compressed unified.blob
const uint8_t flash_blob[799] PROGMEM = {};
#endif
...
uint16_t Write_BLOB_to_Flash_Sector_0(uint16_t FWol, uint32_t First_Unused_RAM_G_Address, uint32_t *Flash_Sector)
{
	...
	uint32_t End_RAM_G_Address;
	End_RAM_G_Address = First_Unused_RAM_G_Address;
	FWol = EVE_Inflate_to_RAM_G(FWol, flash_blob, sizeof(flash_blob), &End_RAM_G_Address);
...
{
	// Outline:
	//  * Read the compressed, 256-byte sectors from the uSD and inflate them
	//  into RAM_G
	//  * Figure out how big the resulting data in RAM_G is
	//  * Write the RAM_G data to flash, using 4096 byte sectors.
	//
	// This will use as much RAM_G as the file inflates to, but as soon as it is
	// written to flash, the RAM_G is no longer needed and can be re-used.
	//  DBG_GEEK("\nInflate_uSD_File_To_Flash initial RAM_G: 0x%08lX =
	//  %lu\n",First_Unused_RAM_G_Address,First_Unused_RAM_G_Address);
	...
	FWol = Start_Busy_Spinner_Screen(FWol,
	    ...
	    // spinner color
	    EVE_ENC_COLOR_RGB(0x00, 0xFF, 0x00), F("Reading uSD, writing to flash: \"BLUEMARB.AZ\" . . ."));

	// Make sure the EVE is caught up from any previous commands
	FWol = Wait_for_EVE_Execution_Complete(FWol);

	File binary_file = SD.open(File_Name, FILE_READ);
	if (0 == binary_file)
	{
	...
	// We are assuming that the entire inflated data will fit into RAM_G.
...
#define CHUNK_SIZE (256)
	...
	*Data_Length = Final_RAM_G_Address - First_Unused_RAM_G_Address;
	...
	//  DBG_GEEK("Writing %ld bytes to flash sector = %ld, flash address = %ld
	//  RAM_G = %ld. .
	//  .",flash_length,*Flash_Sector,(*Flash_Sector)<<12,First_Unused_RAM_G_Address);
	FWol = EVE_Cmd_Dat_3(FWol,
	    // Command
	    EVE_ENC_CMD_FLASHUPDATE,
	    // destination in flash (must be 4096 byte aligned)
	...
	do
	{
		...
		if (4096 <= flash_sector_bytes)
		{
			...
			FWol = EVE_Cmd_Dat_3(FWol,
			    ...
			    // Size (must be multiple of 4096)
			    4096);
			// Update the ring buffer pointer so the coprocessor starts
			// executing
			EVE_REG_Write_16(EVE_REG_CMD_WRITE, (FWol));
...
{
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_HANDLE(handle));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_SOURCE(source));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_EXT_FORMAT(format));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_LAYOUT_H(colstride >> 10, rowstride >> 9));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_LAYOUT(EVE_GLFORMAT, colstride, rowstride));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_SIZE_H(width >> 9, height >> 9));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_SIZE(filter, wrapx, wrapy, width, height));
	return (FWol);
}
