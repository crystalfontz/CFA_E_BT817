...
{
	uint32_t Flash_Sector_Marble;
	uint32_t Flash_Length_Marble;
	uint32_t Flash_Sector_Splash;
	uint32_t Flash_Length_Splash;
	uint32_t Flash_Sector_Clouds;
	uint32_t Flash_Length_Clouds;
	uint32_t Flash_Sector_ICE_FPV_512x300;
	uint32_t Flash_Length_ICE_FPV_512x300;
	...
	FWol = Write_BLOB_to_Flash_Sector_0(FWol, RAM_G_Unused_Start, Flash_Sector);
	DBG_GEEK("done.\n");
	...
	// starts so it does not overwrite anything, but only uses RAM_G
	// temporarily, so it does not change RAM_G_Unused_Start.
	FWol = Inflate_uSD_File_To_Flash(FWol, "bluemarb.a8z", RAM_G_Unused_Start, Flash_Sector, &Flash_Length_Marble);
	DBG_GEEK("done.\n");
	...
	// starts so it does not overwrite anything, but only uses RAM_G
	// temporarily, so it does not change RAM_G_Unused_Start.
	FWol = Inflate_uSD_File_To_Flash(FWol, "splash.a8z", RAM_G_Unused_Start, Flash_Sector, &Flash_Length_Splash);
	DBG_GEEK("done.\n");
	...
	// starts so it does not overwrite anything, but only uses RAM_G
	// temporarily, so it does not change RAM_G_Unused_Start.
	FWol = Inflate_uSD_File_To_Flash(FWol, "cloud.a8z", RAM_G_Unused_Start, Flash_Sector, &Flash_Length_Clouds);
	DBG_GEEK("done.\n");
	...
	// starts so it does not overwrite anything, but only uses RAM_G
	// temporarily, so it does not change RAM_G_Unused_Start.
	FWol = Write_uSD_File_To_Flash(FWol, "ICE_400.avi", RAM_G_Unused_Start, Flash_Sector, &Flash_Length_ICE_FPV_512x300);
	DBG_GEEK("done.\n");

	// DBG_GEEK("Writing \"BBB_1024.avz\" to flash sector 0x%04lX = %ld . . .
	// ",*Flash_Sector); Flash_Sector_BBB1024x600=*Flash_Sector;
	// //Write_uSD_File_To_Flash() needs to know where unused RAM_G
	// //starts so it does not overwrite anything, but only uses RAM_G
	// //temporarily, so it does not change RAM_G_Unused_Start.
	...
	// Dump the flash sectors to the console, so we can copy those into the
	// source and use them to access the flash. Yes, I realize this is hokey,
	// properly we could write some kind of file system or at least a directory
	// to the flash.
	...
	// DBG_STAT("#define FLASH_SECTOR_BBB1024x600
	// (%ldUL)\n",Flash_Sector_BBB1024x600); DBG_STAT("#define
	// FLASH_LENGTH_BBB1024x600 (%ldUL)\n\n",Flash_Length_BBB1024x600);
...
uint16_t Add_Touch_Dot_To_Display_List(uint16_t FWol, uint16_t touch_x, uint16_t touch_y)
{
	...
	if (touch_x < 1200)
	{
		...
		FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_COLOR_RGB(0x00, 0xFF, 0x00));
		// Make it solid
		FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_COLOR_A(0xFF));

		// Draw the touch dot -- a 60px point (filled circle)
		FWol = EVE_Point(FWol, touch_x * 16, touch_y * 16, 60 * 16);
	}
	// Give the updated write pointer back to the caller
	return (FWol);
}
...
int16_t background_slide;
uint8_t background_slide_slow;
uint8_t background_bitmap_handle;
//---------------------------------------------------------------------------
...
uint16_t Add_Bitmap_To_Display_List(uint16_t FWol)
{
	...
	// The first tile starts at up to LCD_WIDTH pixels to the left of the
	// visible display.
	...
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_HANDLE(background_bitmap_handle));
	...
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_VERTEX2F((tile_offset) * 16, 0 * 16));
...
int32_t marble_x_pos;
int32_t marble_x_vel;
int32_t marble_y_pos;
int32_t marble_y_vel;
int32_t marble_rotation;
int32_t marble_spin;
uint32_t Marble_RAM_G_Address;
uint32_t marble_width;
uint32_t marble_height;
uint8_t marble_bitmap_handle;
//---------------------------------------------------------------------------
// Requires uSD
uint16_t Initialize_Marble_Demo(uint16_t FWol, uint32_t *RAM_G_Unused_Start, uint8_t next_bitmap_handle_available)
{
	...
	//                     uint32_t src,
	//                     uint32_t num);
	//  void Gpu_CoCmd_FlashRead(Gpu_Hal_Context_t *phost,uint32_t dest,
	...
	FWol = EVE_Cmd_Dat_3(FWol, EVE_ENC_CMD_FLASHREAD,
...
uint16_t Add_Marble_To_Display_List(uint16_t FWol)
{
	...
	// Make sure the EVE has the idea of which bitmap they it
	// should be working on.
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_HANDLE(marble_bitmap_handle));
	...
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_HANDLE(11));

	// DBG_GEEK("Marble_RAM_G_Address: %lu\n",Marble_RAM_G_Address);

#if 1
	FWol = EVE_Cmd_Dat_3(FWol, EVE_ENC_CMD_SETBITMAP, Marble_RAM_G_Address,
	    // Width (high word), Format (low word)
	    //                     ((uint32_t)marble_width << 16) |
	    //                     EVE_FORMAT_COMPRESSED_RGBA_ASTC_4x4_KHR,
	    ((uint32_t)marble_width << 16) | EVE_FORMAT_COMPRESSED_RGBA_ASTC_8x8_KHR,
	    // Height (low word) (high word unused)
	    marble_height); // height
#else
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_SOURCE(Marble_RAM_G_Address));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_LAYOUT(EVE_FORMAT_ARGB1555, marble_width * 2, marble_height));

	FWol = EVE_Cmd_Dat_0(FWol,
	    EVE_ENC_BITMAP_SIZE(EVE_FILTER_NEAREST, // EVE_FILTER_BILINEAR, //BiLinear is much more
	                                            // work
	        EVE_WRAP_BORDER, EVE_WRAP_BORDER, marble_width, marble_height));
#endif
	// Rotate the bitmap
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_CMD_LOADIDENTITY);
	// Translate to the center
	FWol = EVE_Cmd_Dat_2(FWol, EVE_ENC_CMD_TRANSLATE, +(to_16_16_fp(marble_width, 0) / 2), +(to_16_16_fp(marble_height, 0) / 2));
	// The actual rotate command
	FWol = EVE_Cmd_Dat_1(FWol, EVE_ENC_CMD_ROTATE, to_16_16_fp(marble_rotation, 0) / 360);
	// Undo the translation
	FWol = EVE_Cmd_Dat_2(FWol, EVE_ENC_CMD_TRANSLATE, -(to_16_16_fp(marble_width, 0) / 2), -(to_16_16_fp(marble_height, 0) / 2));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_CMD_SETMATRIX);
	// Render the bitmap it to the current frame
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_VERTEX2F((marble_x_pos - (marble_width * (16 / 2))), (marble_y_pos - (marble_height * (16 / 2)))));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_END());
...
void Move_Marble(void)
{
	...
	uint16_t move_marble_width;
	move_marble_width = marble_width * (16 / 2);
	uint16_t move_marble_height;
...
uint8_t transparency_direction;
//---------------------------------------------------------------------------
...
uint16_t Add_Bounce_To_Display_List(uint16_t FWol)
{
	...
	//========== RUBBER BAND TETHER ==========
	// Draw the rubberband.
	// Maximum stretched would be LCD_WIDTH/2 + LCD_WIDTH/2 (manhatten) make
	// that 1 pixels wide, make the minimum 10 pixels wide
	...
	if (rubberband_width & 0x8000) rubberband_width = 1;
...
uint16_t logo_rotate_degrees;
// Used to pause the logo between spinning sesions
uint16_t logo_rotate_pause;
uint8_t logo_bitmap_handle;
...
uint16_t Initialize_Logo_Demo(uint16_t FWol, uint32_t *RAM_G_Unused_Start, uint8_t next_bitmap_handle_available)
{
	...
	// indexed color PNGs.
	FWol = EVE_Load_PNG_to_RAM_G(FWol, CFA_E_PNG_LOGO, LOGO_SIZE_PNG, RAM_G_Unused_Start, &Logo_Width, &Logo_Height);
	...
	FWol = EVE_Inflate_to_RAM_G(FWol, CFA_E_ARGB2_LOGO, LOGO_SIZE_ARGB2, RAM_G_Unused_Start);
...
uint16_t Add_Logo_To_Display_List(uint16_t FWol)
{
	...
	// Make sure the EVE has the idea of which bitmap they it
	// should be working on.
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_HANDLE(logo_bitmap_handle));
  ...
  FWol=EVE_Cmd_Dat_0(FWol, EVE_ENC_BITMAP_LAYOUT(EVE_FORMAT_ARGB4,Logo_Width*2,Logo_Height));
	...
	// The actual rotate command
	FWol = EVE_Cmd_Dat_1(FWol, EVE_ENC_CMD_ROTATE, to_16_16_fp(logo_rotate_degrees, 0) / 360);
	// Undo the translation, and move to the center of the screen.
	FWol =
	    EVE_Cmd_Dat_2(FWol, EVE_ENC_CMD_TRANSLATE, (to_16_16_fp(-Logo_Width / 2 + 0 / 2, 0)), (to_16_16_fp(-Logo_Height / 2 + 0 / 2, 0)));
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_CMD_SETMATRIX);

	// Render the bitmap to the current frame
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_VERTEX2F((LCD_WIDTH - Logo_Width) * (16 / 2), (LCD_HEIGHT - Logo_Height) * (16 / 2)));
...
uint32_t RAM_G_video_completion_flag_Address;
uint32_t RAM_G_bitmap_Address;
uint8_t video_bitmap_handle;
//---------------------------------------------------------------------------
...
uint16_t Initialize_Video_Demo(uint16_t FWol, uint32_t *RAM_G_Unused_Start, uint8_t next_bitmap_handle_available)
{
	...
	*RAM_G_Unused_Start += (FLASH_VIDEO_WIDTH * 2L) * FLASH_VIDEO_HEIGHT;
	...
	FWol = EVE_Cmd_Dat_1(FWol, EVE_ENC_CMD_FLASHSOURCE, FLASH_SECTOR_ICE_FPV_512x300 << 12);
	// Initialize the video decoder based on the flash address that
	// we just wrote.
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_CMD_VIDEOSTARTF);
...
#endif // (0 != TOUCH_DEMO)
{
	...
	if ((0x01 == points_touched_mask))
	{
		...
		FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_BEGIN(EVE_BEGIN_BITMAPS));
		FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_VERTEX2F(video_x_pos, video_y_pos));
	}
	else
	{
		...
		if (0x03 == points_touched_mask)
		{
			...
			FWol = EVE_Cmd_Dat_2(FWol, EVE_ENC_CMD_SCALE, scale_factor, scale_factor);
...
uint16_t Update_Video_Frame(uint16_t FWol)
{
	...
	if (1 <= video_frame_delay)
	{
		...
		FWol = EVE_Cmd_Dat_2(FWol, EVE_ENC_CMD_VIDEOFRAME,
		    // Memory location to load the frame data, this will be located in
		    // RAM_G.
		    RAM_G_bitmap_Address,
