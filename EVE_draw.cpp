...
{
	// Set the line width (16/16 of a pixel--appears to be about as sharp as it
	// gets)
...
// and the text.
	...
	// void Gpu_CoCmd_Text(Gpu_Hal_Context_t *phost,int16_t x, int16_t y,
	// int16_t font, uint16_t options, const char8_t* s)
	//   {
...
uint16_t EVE_TextF(uint16_t FWol, uint16_t x, uint16_t y, uint16_t Font, uint16_t Options, const __FlashStringHelper *message)
{
    ...
    uint16_t FWol, uint32_t Clear_Color, uint32_t Text_Color, uint32_t Spinner_Color, const __FlashStringHelper *message)
{
	...
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_CLEAR(1 /*CLR_COL*/, 1 /*CLR_STN*/, 1 /*CLR_TAG*/));
	...
	// Display the caller's message at the center of the screen using bitmap
	// handle 25
	FWol = EVE_TextF(FWol, LCD_WIDTH / 2, LCD_HEIGHT / 2, 25, EVE_OPT_CENTER, message);
	// Set the drawing color for the spinner
	FWol = EVE_Cmd_Dat_0(FWol, Spinner_Color);
	// Send the spinner go command
	// This appears to be broken...
	// FWol=EVE_Cmd_Dat_2(FWol, EVE_ENC_CMD_SPINNER, (((uint32_t)LCD_HEIGHT/2)<<16) | (LCD_WIDTH/2), style, (((uint32_t)1)<<16) | (1));
...
uint16_t Calibrate_Touch(uint16_t FWol)
{
	...
	// Clear the screen - this and the previous prevent artifacts between lists
	FWol = EVE_Cmd_Dat_0(FWol, EVE_ENC_CLEAR(1 /*CLR_COL*/, 1 /*CLR_STN*/, 1 /*CLR_TAG*/));
	FWol = EVE_PrintF(FWol, LCD_WIDTH / 2, LCD_HEIGHT / 2, 25, // font
	    EVE_OPT_CENTER, "Touch dot to calibrate");
    ...
    uint16_t FWol, const uint8_t *PNG_data, uint32_t PNG_length, uint32_t *RAM_G_Address, uint32_t *Image_Width, uint32_t *Image_Height)
{
	...
	*RAM_G_Address = (*RAM_G_Address + 0x07) & 0xFFFFFFF8;
	...
	uint32_t Read_Back_RAM_G_First_Available;
	uint32_t Read_Back_Width;
	uint32_t Read_Back_Height;

	// Now go figure out what we did.
	FWol = Get_RAM_G_Properties_After_LOADIMAGE(FWol, &Read_Back_RAM_G_First_Available, &Read_Back_Width, &Read_Back_Height);
	if ((*RAM_G_Address != Read_Back_RAM_G_First_Available) || (*Image_Width != Read_Back_Width) || (*Image_Height != Read_Back_Height))
	{
