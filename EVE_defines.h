...
// BT817 Memory Commands - use with EVE_Command_Write and ft800memReadxx
// BT817 Host Memory Write
#define EVE_MEM_WRITE (0x00000080UL) // = 128UL
// BT817 Host Memory Read
#define EVE_MEM_READ (0x00000000UL) // = 0UL

// Initializes BT817
#define EVE_ACTIVE (0x00000000UL) // = 0UL
// Place BT817 in Standby (clk running)
#define EVE_STANDBY (0x00000041UL) // = 65UL
// Place BT817 in Sleep (clk off)
#define EVE_SLEEP (0x00000042UL) // = 66UL
// Place BT817 in Power Down (core off)
...
#define EVE_CLOCK_SOURCE_INTERNAL (0) // internal clock appears to be unsupported by BT817
...
// For displaying a bitmap stored in flash, addressed by sector.
...
#define FLASH_SETBITMAP(FWO, SECTOR, FORMAT, WIDTH, HEIGHT) \
	EVE_Cmd_Dat_3(FWO, EVE_ENC_CMD_SETBITMAP, ((uint32_t)0x800000) | (((uint32_t)SECTOR) << 7), \
	    (((uint32_t)WIDTH) << 16) | ((uint32_t)FORMAT), (uint32_t)HEIGHT)

// The parameters filter/wrapx/wrapy are always set to NEAREST/BORDER/BORDER
#define RAMG_SETBITMAP(FWO, RAM_G_ADDRESS, FORMAT, WIDTH, HEIGHT) \
	EVE_Cmd_Dat_3(FWO, EVE_ENC_CMD_SETBITMAP, RAM_G_ADDRESS, (((uint32_t)WIDTH) << 16) | ((uint32_t)FORMAT), (uint32_t)HEIGHT)

// ##################################################################################################
//  Below are defines as provided by FTDI / BridgeTek, refactored / cleaned up a bit
//  by Crystalfontz.
// ## COMMOM ########################################################################################
...
#define EVE_ENC_VERTEX2II(x, y, handle, cell) \
	((2UL << 30) | (((x) & 0x01FFUL) << 21) | (((y) & 0x01FFUL) << 12) | (((handle) & 0x01FUL) << 7) | (((cell) & 0x07FUL) << 0))
// This throws "warning: comparison of unsigned expression < 0 is always false"
// #define EVE_ENC_BITMAP_SOURCE(addr)                          (( 1UL<<24)|((addr) < 0 ?  (((addr)&0x007FFFFFUL)) :
// ((addr)&0x00FFFFFFUL))) My edit:
...
#define EVE_ENC_CLEAR_COLOR_RGB(red, green, blue) \
	((2UL << 24) | (((red) & 0x0FFUL) << 16) | (((green) & 0x0FFUL) << 8) | (((blue) & 0x0FFUL) << 0))
#define EVE_ENC_TAG(s) ((3UL << 24) | (((s) & 0x0FFUL) << 0))
#define EVE_ENC_COLOR_RGB(red, green, blue) \
	((4UL << 24) | (((red) & 0x0FFUL) << 16) | (((green) & 0x0FFUL) << 8) | (((blue) & 0x0FFUL) << 0))
...
#define EVE_ENC_BITMAP_LAYOUT(format, linestride, height) \
	((7UL << 24) | (((format) & 0x031FUL) << 19) | (((linestride) & 0x03FFUL) << 9) | (((height) & 0x01FFUL) << 0))
#define EVE_ENC_BITMAP_SIZE(filter, wrapx, wrapy, width, height) \
	((8UL << 24) | (((filter) & 0x01UL) << 20) | (((wrapx) & 0x01UL) << 19) | (((wrapy) & 0x01UL) << 18) | (((width) & 0x01FFUL) << 9) | \
	    (((height) & 0x01FFUL) << 0))
#define EVE_ENC_ALPHA_FUNC(func, ref) ((9UL << 24) | (((func) & 0x07UL) << 8) | (((ref) & 0x0FFUL) << 0))
#define EVE_ENC_STENCIL_FUNC(func, ref, mask) \
	((10UL << 24) | (((func) & 0x07UL) << 16) | (((ref) & 0x0FFUL) << 8) | (((mask) & 0x0FFUL) << 0))
...
#define EVE_ENC_COLOR_MASK(r, g, b, a) \
	((32UL << 24) | (((r) & 0x01UL) << 3) | (((g) & 0x01UL) << 2) | (((b) & 0x01UL) << 1) | (((a) & 0x01UL) << 0))
...
#define EVE_ENC_BITMAP_SWIZZLE(r, g, b, a) \
	((47UL << 24) | (((r) & 0x07UL) << 9) | (((g) & 0x07UL) << 6) | (((b) & 0x07UL) << 3) | (((a) & 0x07UL) << 0))
